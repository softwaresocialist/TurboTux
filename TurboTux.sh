...
if [[ "$DISTRO" == "arch" ]]; then
    ...
    # Repos
    if ask_user "Update and enable necessary repositories?"; then
    ...
    if ask_user "Install an RGB control app (OpenRGB)?"; then
     ...
     # mangojuice
    if ask_user "Install a performance monitoring overlay like RivaTunerStatistics/Afterburner (mangojuice/mangohud)?"; then
...
# Ubuntu section
    ...
    # Repos
    if ask_user "Update and setup Flaptak?"; then
      sudo apt update && sudo apt upgrade -y
      sudo apt install -y flatpak
      sudo flatpak remote-add --if-not-exists flathub https://flathub.org/repo/flathub.flatpakrepo
    else
      echo -e "\e[1;31mUpdates and Flatpak are required. Exiting...\e[0m"
      exit 1
    fi
    ...
    # mangojuice
    if ask_user "Install a performance monitoring overlay like RivaTunerStatistics (mangojuice/mangohud)?"; then
...
# OpenSUSE section
    ...
    # Repos
    if ask_user "Update and setup Flatpak ?"; then
